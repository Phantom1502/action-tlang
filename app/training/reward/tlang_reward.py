    ...
    def __call__(
        ...
        **kwargs,
        ...
        for prompt, completion, future_bin in zip(prompts, completions, future_bins):
            future_bin_nodes = [CandleNode(open=c[0], high=c[1], low=c[2], close=c[3]) for c in future_bin]
            reward = self.compute_reward(prompt, completion, future_bin_nodes)
...
if __name__ == "__main__":
    ...
    print("SUPPORT BUFF GROUPS")
    print(cfg.action_buff('round1', 'support'))
    print(cfg.rounds['round1'].zone_buffs['support'].action_buffs['BUY'])
    print(cfg.rounds['round1'].zone_buffs['support'].action_buffs['HOLD'])
    
    print("RESISTANCE BUFF GROUPS")
    print(cfg.rounds['round1'].zone_buffs['resistance'].action_buffs['SELL'])
    print(cfg.rounds['round1'].zone_buffs['resistance'].action_buffs['HOLD'])
