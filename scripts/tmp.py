...
if __name__ == "__main__":
    ...
    df = pd.read_parquet("data/slide_window/pretrain/window_200_val.parquet")
    ...
    noise = (cfg.tlang.zone_range[1] - cfg.tlang.zone_range[0]) // 2
    
    symbol = row["symbol"].split("_")[0]
    timeframe = row["symbol"].split("_")[1]
    input_window = np.array(row["input_window"].tolist())
    future_window = np.array(row["future_window"].tolist())
    atr_100 = row["atr_100"]
        ...
        for future_idx, lower_bin, upper_bin, width in zones:
            ...
            if score > hold_threshold:
                ...
                for _ in range(n_samples):
                    if noise > 0:
                        lower_bin = max(0, zone.lower_bin - random.randint(0, noise))
                        upper_bin = min(cfg.tlang.n_bins, zone.upper_bin + random.randint(0, noise))
    ...
    for score, zone in zone_nodes:
        ...
        if score > trend_threshold and zone.direction == ZoneDirection.support:
            trend = TrendType.UP
        elif score > trend_threshold and zone.direction == ZoneDirection.resistance:
        ...
        print(prompt)
        print(completion)
        plot_program(ProgramNode(chart=chart, think=think, action=action), future_candles=future_candles)
